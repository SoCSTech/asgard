...
export function HeaderLink(props: Props) {
  ...
  if (currentSection === buttonSection) {
    return (
      <li className="text-xl tablet:text-md rounded hover:bg-accent hover:text-accent-foreground my-1 tablet:my-0 tablet:mr-5 px-2 tablet:px-1 tablet:underline bg-white tablet:bg-inherit text-black tablet:text-white">
    ...
    return (
      <li
        className="text-xl tablet:text-md rounded hover:bg-accent hover:text-accent-foreground my-1 tablet:my-0 tablet:mr-5 px-2 tablet:px-1"
      >
        <a href={props.href}>{props.children}</a>
      </li>
    );
